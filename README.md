...
### 0.4.0
- **Expanded metrics harvest** — the client poller now captures the full set of useful fields the Omada client API exposes: signal rank, Wi-Fi generation (Wi-Fi 4/5/6/6E/7 from `wifiMode`), connection uptime, cumulative traffic (up/down bytes), activity, and power-save state — alongside the existing RSSI/SNR/signal/channel/band/rates. Existing databases auto-migrate to add the new columns.
- **Fleet Composition panel** — a new Service Levels panel showing the Wi-Fi-generation mix (how modern your wireless fleet is) and the 2.4/5/6 GHz band split across all clients, as segmented bars, plus a power-save count.
- **Richer Signal & Health table** — the per-client table now shows Wi-Fi generation, connection uptime, and a power-save indicator. Traffic rates are formatted (Kbps→Mbps). The empty Experience column was removed (Omada's API doesn't expose the UI's experience score, only the underlying RSSI/SNR/signal-rank, which we already show).
- **RF context in the AI briefing** — the network-health briefing now receives per-client signal facts (weak clients with their band, Wi-Fi generation, and AP; overall coverage %; fleet Wi-Fi-gen breakdown), so the AI can explain *why* a device struggles (e.g. a distant Wi-Fi 4 IoT device on 2.4 GHz) rather than just flagging that it does, and correlate weak signal with disconnects/roams.
- Removed the now-unused experience field from the metrics store and API responses.

### 0.3.0
- **Client lifecycle telemetry** — connect / disconnect / roam events from the Omada controller are now parsed in full, with the access point resolved by name (`[ap:NAME:MAC]`), SSID, channel, and (for disconnects) session duration. Requires enabling Client Health / Client History in the controller's History Data Retention settings; once on, these flow over syslog.
- **Roaming view** — a dedicated view with three visuals: a handoff map (which APs hand off to which, weighted by frequency), a busiest-paths table, roamiest-devices ranking, and a per-device association timeline (a step-line of which AP a client was on over time). Roam events capture both endpoints (from-AP and to-AP) plus both channels; a one-time startup backfill recovers destinations for events stored before this was added.
- **Service Levels (SLE) dashboard** — service-level metrics rendered as success% + trend timeline + root-cause breakdown: Successful Connects, DHCP Success, Roaming Health, and (when metrics polling is on) Coverage. Plus a Persistently Failing Clients detector (repeated short sessions / disconnect-connect imbalance), surfaced here, in the network-health briefing, and as a chatbot tool.
- **Client metrics poller** — an opt-in background poller (`METRICS_POLL_ENABLED=1`) that snapshots per-client RF/health stats (RSSI, SNR, signal, channel, band, experience, tx/rx rates, AP) from the Omada client API into a time-series table. Configurable interval (`METRICS_POLL_INTERVAL`, default 60s) and retention (`METRICS_RETENTION_HOURS`, default 7 days), with automatic pruning. This captures the signal data that syslog never carries.
- **Signal & Health views** — three complementary RF visualizations on the Service Levels dashboard: a live per-client snapshot table (RSSI colour-coded, collapsible), a signal-distribution histogram (how client-time splits across signal bands, with weak-AP attribution), and a per-client RSSI/SNR time-series chart with AP-association shading (click any client).
- **Signal-based roaming insight** — the association timeline colours each stop by the client's RSSI at that AP, and roam-quality scoring judges whether each roam moved the client to a stronger or weaker AP (using the RSSI just before/after the roam; >6 dB drop flags a suboptimal roam).
- **Coverage SLE** — % of client-samples at or above an RSSI threshold (default −72 dBm, configurable via `?rssi_threshold=`), with per-AP breakdown of where coverage is thin.
- **Tri-state event valence** — every event is classified good / neutral / bad, shown as a coloured edge on event rows with good/neutral/bad filter tabs.
- **Metrics field diagnostic** — `/api/metrics/raw_sample` returns the raw client field names from your controller, for resolving any RF field that doesn't map on your Omada version.
